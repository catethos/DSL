...
// Schema automatically included in prompt
...
// LLM returns fully structured analysis
...
```dsl
type Person {
  name: String    // First in schema
  age: Int        // Second in schema
  email: String   // Third in schema
}
```
...
```dsl
// LLM returns: { "name": "Alice" }
// Missing 'age' field

GetPerson()
// ✗ Runtime Error: Missing required field 'age' in Person
```
...
// LLM returns only title and summary
...
type Summary {
  title: String
  molecularStructure: String  // Too technical
  quantumState: Int           // Unrealistic
}
```
...
type Review {
  text: String
  sentiment: String  // LLM might return anything
}
```
...
// GPT-4 models are better with complex schemas
def Extract(text: String) -> ComplexData {
  model: "gpt-4o-mini"  // Good for structured output
  prompt: "Extract: ${text}"
}

// GPT-3.5 may struggle with very complex schemas
def Extract(text: String) -> ComplexData {
  model: "gpt-3.5-turbo"  // May need simpler schema
  prompt: "Extract: ${text}"
}
```
