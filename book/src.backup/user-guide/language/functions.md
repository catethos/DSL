...
classify_value(-42)
// Returns: { value: -42, absolute: 42, category: "medium" }
```
...
make_user("Alice", 30)
// Returns: { name: "Alice", age: 30, is_adult: true }
```
...
```dsl
def compute(x) {
    let temp = x * 2        // Local to compute()
    let result = temp + 1   // Local to compute()
    result
}

compute(5)      // Returns 11
// temp         // ✗ Error: temp not defined
// result       // ✗ Error: result not defined
```
...
def scale(x) {
    x * multiplier      // Accesses global 'multiplier'
}
...
validate_age(-5)    // "Age cannot be negative"
```
...
// Use multi-statement for clarity
def process(data) {
...
def increment() {
    counter = counter + 1  // ✗ Error: variables are immutable
}

// DSL functions are pure - they cannot mutate global state
```
