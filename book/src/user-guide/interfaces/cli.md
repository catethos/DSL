...
generate_data | dsl -c <<'DSL' > output.json
  SQL("SELECT * FROM stdin")
    |> filter(_, fn row => row.value > 100 end)
    |> map(_, fn row => {id: row.id, value: row.value} end)
DSL
```
...
def getUsers() { SQL("SELECT * FROM 'users.csv'") }
    |> filter(_, fn u => u.active end)
    |> map(_, fn u => {id: u.id, name: u.name} end)
...
def main() { SQL("SELECT * FROM 'tasks.csv'") }
    |> filter(_, fn t => t.status == "pending")
    |> map(_, fn t => "- " + t.name end)
    |> Join(_, "\n")
...
def processSales(filename) { SQL("SELECT * FROM '" + filename + "'") }
    |> filter(_, fn s => s.amount > 0 end)
    |> map(_, fn s => {
         category: s.category,
         amount: s.amount,
         month: Split(s.date, "-" end)[1]
       })
    |> GroupBy(_, "category")
    |> map(_, fn group => {
         category: group.key,
         total: Sum(map(group.values, fn s => s.amount end)),
...
def assert(condition, message) { condition }
    ? "✓ " + message
    : Error("✗ " + message)
