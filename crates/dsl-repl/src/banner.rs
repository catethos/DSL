...
pub fn get_banner() -> Vec<Line<'static>> {
    vec![
        ...
        Line::from(vec![
            Span::styled("  → ", Style::default().fg(Color::Blue)),
            Span::styled("let y = 10", Style::default().fg(Color::Magenta)),
            Span::styled(
                "      # Alternative syntax",
                Style::default().fg(Color::DarkGray),
            ),
        ...
        Line::from(vec![
            Span::styled("  → ", Style::default().fg(Color::Blue)),
            Span::styled("x + y", Style::default().fg(Color::Magenta)),
            Span::styled(
                "          # Use in expression",
                Style::default().fg(Color::DarkGray),
            ),
...
pub fn get_help_text() -> Vec<String> {
    vec![
        ...
        "  Binding:     expr as varname  OR  let varname = expr".to_string(),
        "  Access:      list[0], obj.field".to_string(),
        "  Special:     _  (last result)".to_string(),
        "".to_string(),
        "Examples:".to_string(),
        "  5 as x           # Bind 5 to variable x".to_string(),
        "  let y = 10       # Alternative binding syntax".to_string(),
        "  x + y            # Use variables in expression".to_string(),
